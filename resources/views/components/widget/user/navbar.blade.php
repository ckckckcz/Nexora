            ...
            <div class="flex space-x-2">
                @auth 
                    @if (Auth::user()->role == 'admin')
                        ...
                        <p class="text-[#1A3C34]  font-medium px-3 py-2 w-1/2 text-center">{{ Auth::user()->username }}</p>
                    @elseif (Auth::user()->role == 'dosen')
                    <p class="text-[#1A3C34]  font-medium px-3 py-2 w-1/2 text-center">{{ Auth::user()->dosen->nama_dosen }}</p>
                    
                    @elseif (Auth::user()->role == 'mahasiswa')
                    <p class="text-[#1A3C34]  font-medium px-3 py-2 w-1/2 text-center">{{ Auth::user()->mahasiswa->nama_mahasiswa }}</p>
                    @endif
                    <a href="{{url('/logout')}}" class="text-[#1A3C34] hover:bg-gray-100 font-medium px-3 py-2 border border-gray-200 rounded-lg w-1/2 text-center">Logout</a>
                @else
