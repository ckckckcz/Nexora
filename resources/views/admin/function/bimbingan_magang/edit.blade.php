...
@section('admin')
    <div class="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
        <header class="mb-8">
            <h1 class="text-3xl font-bold text-blue-900">Edit Data Bimbingan Magang 📝</h1>
        </header>

        <section class="bg-white rounded-2xl border border-gray-200 transition-shadow duration-300 hover:shadow-lg">
            <div class="p-4 sm:p-6 flex flex-col gap-6">
                <form action="/admin/bimbingan-magang/edit/{{$bimbinganMagang->id_bimbingan}}" method="POST" class="flex flex-col gap-6">
                    @csrf
                    @method('PUT')
                    <!-- Mahasiswa -->
                        ...
                        <div class="relative">
                            <select id="id_mahasiswa" name="id_mahasiswa"
                                ...
                                @foreach($mahasiswas as $mahasiswa)
                                    <option value="{{ $mahasiswa->id_mahasiswa }}" {{ old('id_mahasiswa', $mahasiswa->id_mahasiswa) == $bimbinganMagang->id_mahasiswa ? 'selected' : '' }}>{{ $mahasiswa->nama_mahasiswa }} ({{ $mahasiswa->nim }})</option>
                                @endforeach
                            </select>
                            <div
                        ...
                        <div class="relative">
                            <select id="id_dosen" name="id_dosen"
                                ...
                                @foreach($dosens as $dosen)
                                    <option value="{{ $dosen->id_dosen }}" {{ old('id_dosen', $dosen->id_dosen) == $bimbinganMagang->id_dosen ? 'selected' : '' }}>{{ $dosen->nama_dosen }} ({{ $dosen->nidn }})</option>
                                @endforeach
                            </select>
                            <div
                                class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                        ...
                        <div class="relative">
                            <select id="id_lowongan_magang" name="id_lowongan_magang"
                                ...
                                @foreach($lowongans as $lowongan)
                                    <option value="{{ $lowongan->id_lowongan }}" {{ old('id_lowongan', $lowongan->id_lowongan) == $bimbinganMagang->id_lowongan_magang ? 'selected' : '' }}>{{ $lowongan->nama_perusahaan }} - {{ $lowongan->posisiMagang->nama_posisi }}</option>
                                    </option>
                                @endforeach
                            </select>
                            <div
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="relative">
                            <select id="status_bimbingan" name="status_bimbingan"
                                ...
                                <option value="aktif" {{ old('status_bimbingan', $bimbinganMagang->status_bimbingan) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="selesai" {{ old('status_bimbingan', $bimbinganMagang->status_bimbingan) == 'selesai' ? 'selected' : '' }}>Selesai</option>
                            </select>
                            <div
                                class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                <svg class="fill-current h-4 w-4 transition-transform duration-200"
