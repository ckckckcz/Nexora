...
@section('admin')
    <div class="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
        <header class="mb-8">
            <h1 class="text-3xl font-bold text-blue-900">Edit Data Posisi Magang 📅</h1>
        </header>
        ...
        <section class="bg-white rounded-2xl border border-gray-200 transition-shadow duration-300 hover:shadow-lg">
            <div class="p-4 sm:p-6 flex flex-col gap-6">
                <form action="/admin/posisi-magang/edit/{{ $posisi->id_posisi_magang }}" method="POST" class="flex flex-col gap-6">
                    @csrf
                    @method('PUT')
                     <div class="flex flex-col gap-2">
                        ...
                        <input type="text" name="nama_posisi" id="nama_posisi"
                            ...
                            placeholder="Masukkan nama posisi magang" value="{{ old('nama_posisi', $posisi->nama_posisi)}}"  required>
                        @error('nama_posisi')
